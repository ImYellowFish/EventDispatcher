...
// Ajust Hue, Saturation, Brightness and Saturation
// Hue is disabled for performance
inline fixed3 HSBC(half3 startColor, fixed4 hsbc, half2 uv)
{
	...
	outputColor = (outputColor - 0.5f) * (_Contrast)+0.5f;
	outputColor = outputColor + _Brightness;
	fixed3 intensity = dot(outputColor, float3(0.299, 0.587, 0.114));
	outputColor = lerp(intensity, outputColor, _Saturation);
	outputColor = lerp(startColor, outputColor, tex2D(_HsbcMask, uv).r);

	return outputColor;
}


inline fixed3 ApplyHue(float3 aColor, float aHue)
{
...
// uv flow
fixed4 _UVFlow_Color;
fixed4 _UVFlow_Speed;
sampler2D _UVFlow_Tex;
fixed4 _UVFlow_Tex_ST;

inline fixed3 UV_Flow(half4 tex) {
	fixed3 mask = tex2D(_EmissionMap, tex.xy);
	tex.xy = TRANSFORM_TEX(tex, _UVFlow_Tex) + _UVFlow_Speed.xy * _Time.y;
	return _UVFlow_Color.rgb * mask.rgb * tex2D(_UVFlow_Tex, tex.xy).rgb;
}
#endif
...
#ifdef _ZGAME_SUBSURFACE_SCATTERING
	// apply subsurface scattering for forward add pass
	c.rgb = c.rgb + SSS(light.dir, s.normalWorld, -s.eyeVec, light.color, i.tex.xy);
#endif
